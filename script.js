  ...
  /* ---- Downloadable policy PDF ----
     Built from the policy on the page so it stays in sync with the wording shown.
     The "Requirements from Finance" section (#financeReq) is deliberately EXCLUDED —
     it is an internal process notice, not part of the policy document. */
  function generatePolicyPDF() {
    if (!window.jspdf || !window.jspdf.jsPDF) {
      showToast('The PDF library hasn\u2019t loaded. Open the app in a browser and try again.');
      return;
    }
    ...
    function newPageIfNeeded(need) {
      if (y + need > pageH - 60) { doc.addPage(); y = 56; }
    }

    // Header
    if (typeof LOGO_DATA !== 'undefined' && LOGO_DATA) {
      try { doc.addImage(LOGO_DATA, 'PNG', M, y, 40, 40); } catch (e) {}
    }
    doc.setFont('helvetica', 'bold'); doc.setFontSize(14);
    doc.text('Master Drilling South Africa', M + 52, y + 14);
    doc.setFontSize(12);
    doc.text('Disbursement Policy', M + 52, y + 31);
    y += 56;
    doc.setDrawColor(200); doc.line(M, y, pageW - M, y); y += 22;

    // Body, read from the general policy card only
    const sections = document.querySelectorAll('#view-manual .policy:not(#financeReq)');
    sections.forEach(sec => {
      Array.from(sec.children).forEach(el => {
        ...
        if (tag === 'h3') {
          ...
          doc.text(el.innerText.trim(), M, y);
          y += 16;
        } else if (tag === 'p') {
          doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(60);
          const lines = doc.splitTextToSize(el.innerText.trim(), W);
          ...
          Array.from(el.children).forEach((li, i) => {
            ...
            const lines = doc.splitTextToSize(li.innerText.trim(), W - 20);
            newPageIfNeeded(lines.length * 13 + 6);
            doc.text(bullet, M + 4, y);
    ...
    for (let p = 1; p <= pages; p++) {
      ...
      doc.text('Master Drilling — Disbursement Policy', M, pageH - 30);
      doc.text('Page ' + p + ' of ' + pages, pageW - M, pageH - 30, { align: 'right' });
    }

    try {
      doc.save('MDG-Disbursement-Policy.pdf');
      showToast('Policy downloaded. If nothing started, this preview may be blocking it — open the app in a browser.', 6000);
    } catch (e) {
      try { window.open(doc.output('bloburl'), '_blank'); } catch (e2) {}
      showToast('Your policy PDF is ready, but this preview blocked the download.', 6000);
    }
  }
  const policyPdfBtn = document.getElementById('policyPdfBtn');
  if (policyPdfBtn) policyPdfBtn.addEventListener('click', generatePolicyPDF);
